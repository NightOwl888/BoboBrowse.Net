...
powershell -ExecutionPolicy Bypass -Command "& Import-Module .\build\psake.psm1; $psake.use_exit_on_error = $true; Invoke-psake .\build\runbuild.ps1 -properties @{version='%version%';configuration='%configuration%';packageVersion='%PackageVersion%'}"
