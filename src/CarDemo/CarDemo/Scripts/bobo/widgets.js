...
function SelectionWidget(){
	...
	this.update=function(selections,choices){
    	...
    	var choiceArray = choiceObj != null ? choiceObj.ChoiceList : new Array();
		var choiceTotalCount = choiceObj != null ? choiceObj.TotalCount : 0;
		...
		for (var i = 0; i < choiceArray.length; ++i) {
		    val = choiceArray[i].Value;
		    hits = choiceArray[i].FacetValueHitCount;
...
function tagSort(tag1,tag2){
	s1=tag1.Value.toLowerCase();
	s2=tag2.Value.toLowerCase();
...
function TagCloud(section,callback){
	...
	this.update=function(selections,choices){
		...
		if (choiceArray!=null && choiceArray.length>0){
		    largest = choiceArray[0].FacetValueHitCount;
			smallest = choiceArray[choiceArray.length - 1].FacetValueHitCount;
			...
			for (var i=0;i<choiceArray.length;++i){
				...
				var text=choiceArray[i].Value;
				var weight = choiceArray[i].FacetValueHitCount;
	...
	this.update = function (hits, offset) {
		...
		for (var i = 0; i < hits.length; ++i) {
			...
			for (var k = 0; k < this.fieldnames.length; ++k) {
			    ...
			    var fieldval = doc[fieldName];

			    if (fieldName == "price") {
			        fieldval = formatCurrency(fieldval);
			    } else if (fieldName == "year") {
			        fieldval = formatYear(fieldval);
			    } else if (fieldName == "mileage") {
			        fieldval = formatMileage(fieldval);
			    }
